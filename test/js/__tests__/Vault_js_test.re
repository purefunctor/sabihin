...
describe("Master Key", () => {
  testPromise("it wraps and unwraps a protection private key", () => {
    ...
    let* salt_buffer = Salt.compute_digest(client_random);
    let* master_key = MasterKey.create(salt_buffer);
    let* protection_key_pair = ProtectionKeyPair.create();
    ...
    let* _ =
      Operations.unwrap_protection_private_key(
        master_key.master_key,
        master_key.protection_key_iv,
  ...
  testPromise("it wraps and unwraps a verification private key", () => {
    ...
    let* salt_buffer = Salt.compute_digest(client_random);
    let* master_key = MasterKey.create(salt_buffer);
    let* verification_key_pair = VerificationKeyPair.create();
    let* encrypted_private_key =
    ...
    let* _ =
      Operations.unwrap_verification_private_key(
        master_key.master_key,
        master_key.verification_key_iv,
        encrypted_private_key,
      );
...
describe("Protection Key Pair", () => {
  testPromise("it wraps and unwraps an ephemeral key", () => {
    let* key_pair = ProtectionKeyPair.create();
    let* ephemeral_key = EphemeralKey.create();
    let* encrypted_ephemeral_key =
      Operations.wrap_ephemeral_key(
        key_pair.public_key,
        ephemeral_key.ephemeral_key,
      );
    let* _ =
      Operations.unwrap_ephemeral_key(
...
describe("Verification Key Pair", () => {
  testPromise("it signs and verifies a message", () => {
    ...
    let* verification_key_pair = VerificationKeyPair.create();
    let* signature =
      Operations.sign_data(verification_key_pair.private_key, encoded_data);

    let* success =
      Operations.verify_data(
...
describe("Ephemeral Key", () => {
  testPromise("it encrypts and decrypts a message", () => {
    ...
    let* ephemeral_key = EphemeralKey.create();
    let* encrypted_data =
      Operations.encrypt_data(
        ephemeral_key.ephemeral_key,
        ephemeral_key.message_iv,
        encoded_data,
      );
    let* decrypted_data =
      Operations.decrypt_data(
        ephemeral_key.ephemeral_key,
        ephemeral_key.message_iv,
        encrypted_data,
      );
    let decrypted_data = Uint8Array.fromBuffer(decrypted_data, ());
    let decrypted_message = TextDecoder.decode(text_decoder, decrypted_data);
...
describe("Base64Utils", () => {
  testPromise("it works round trip", () => {
    ...
    let* fresh_derived_key = DerivedKey.create("password", salt_buffer);
